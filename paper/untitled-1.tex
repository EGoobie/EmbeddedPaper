...
The entirety of the signal processing was done on the FPGA with the microprocessor responsible for supplying varioius parameters, most from user input via the front panel in labView shown in Fig. \ref{fig_AEPUfp}. 
...
The FPGA was run at its maximum clock frequency of 40MHz, however the audio processing was done in a timed loop sampling at 40kHz, which is roughly twice the maximum audible frequency for humans meaning the audio signal sound quality should be minimally affected \cite{audio_fundamentals}.
...
The last step in the signal path of the AEPU was the master volume control shown in Fig. \ref{fig_volume}.
